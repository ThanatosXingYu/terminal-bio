...
describe("Terminal Component", () => {
  ...
  describe("Input Features & Initial State", () => {
    it("should render the macOS terminal window shell", () => {
      const terminalWindow = screen.getByRole("region", {
        name: "Terminal window",
      });

      expect(terminalWindow).toBeInTheDocument();
      expect(screen.getByTestId("terminal-titlebar").parentElement).toBe(
        terminalWindow
      );
  ...
  describe("Input Commands", () => {
    ...
    it("should return the fallback visitor identity for 'whoami'", async () => {
  ...
  describe("Invalid Arguments", () => {
    ...
    specialUsageCmds.forEach(cmd => {
      ...
      it(`should return usage component for '${cmd}' cmd with incorrect option`, async () => {
        const arg = cmd === "themes" ? "go light" : "set 1";
        window.open = vi.fn();
